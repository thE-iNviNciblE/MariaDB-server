...
public:
  ...
  */
  ...
  bool is_not_empty_string() const { return m_value && m_value[0]; }
...
{
  Var m_connection_name; // MariaDB: e.g. 'master1'
  Var m_master;          // e.g. 'localhost'
  Var m_filename;        // e.g. 'source-bin.000002'
  Var m_position;        // a number
  Var m_gtid_executed;   // MySQL56: e.g. single '<UUID>:1-5" or multiline
                         // '<UUID1>:1-10,\n<UUID2>:1-20\n<UUID3>:1-30'
  Var m_using_gtid;      // MariaDB: 'No','Slave_Pos','Current_Pos'
...
public:
  ...
  Show_slave_status()
   :m_connection_name("Connection_name"),
    ...
    m_gtid_executed("Executed_Gtid_Set"),
    m_using_gtid("Using_Gtid")
  { }

  void init(MYSQL_RES *res, MYSQL_ROW row)
  {
    m_connection_name.init(res, row);
    m_master.init(res, row);
    m_filename.init(res, row);
    m_position.init(res, row);
    m_gtid_executed.init(res, row);
    m_using_gtid.init(res, row);
    // Normalize
    if (m_gtid_executed.value())
      m_gtid_executed.replace('\n', ' ');
  }
  ...
  bool is_using_gtid() const
  {
    return !m_using_gtid.eq_value("No", 2);
  }
  ...
  {
    if (m_connection_name.is_not_empty_string())
      return m_connection_name.print_quoted(to) || to->append(' ');
    return false;
  }
  ...
  bool print_comment_master_identity(String *comment) const
  {
    if (comment->append("master "))
      return true;
    if (m_connection_name.is_not_empty_string())
      return m_connection_name.print_quoted(comment);
  ...
  bool print_mysql56(String *sql, String *comment) const
  {
    ...
    */
    return
      ...
      m_gtid_executed.print_quoted(sql) ||
      sql->append(";\n") ||
      sql->append("CHANGE MASTER TO MASTER_AUTO_POSITION=1;\n") ||
      print_comment_master_identity(comment) ||
      comment->append(" purge list ") || m_gtid_executed.print_quoted(comment);
  }
  ...
  {
    if (m_gtid_executed.is_not_empty_string())
    {
      /* MySQL >= 5.6 with GTID enabled */
      return print_mysql56(sql, comment);
    }

    if (gtid_slave_pos.is_not_empty_string() && is_using_gtid())
    {
